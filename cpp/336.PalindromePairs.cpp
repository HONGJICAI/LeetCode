    ...
    vector<vector<int>> palindromePairs(vector<string>& words) {
        ...
        for(int i=0;i<words.size();++i)
            for(int j=0;j<words.size();++j)
                if(i==j)
                    continue;
                else if(auto concat=words[i]+words[j];concat==string(rbegin(concat),rend(concat)))
                    ret.push_back({i,j});
        return ret;
    }
};
class Solution {
//Runtime: 152 ms, faster than 94.79% of C++ online submissions for Palindrome Pairs.
//Memory Usage: 28.2 MB, less than 94.95% of C++ online submissions for Palindrome Pairs.
public:
    vector<vector<int>> palindromePairs(vector<string>& words) {
        ...
        for(int i=0;i<words.size();++i){
            ...
            for(int j=0;j<=s.size();++j){
                ...
                if(palindrome(suf)){
                    ...
                    if(auto it=hashmap.find(reverseStr);it!=hashmap.end()&&it!=cur)
