class Solution {
  ...
  piles[n-2], he picks first piles[0], then Lee can pick either piles[1] or
  piles[n - 1]. Every turn, Alex can always pick even indexed piles and Lee can
  only pick odd indexed piles.
  ...
  */
  ...
  // Memory Usage: 8.7 MB, less than 58.60% of C++ online submissions for Stone
  // Game.
public:
  ...
  bool stoneGame(vector<int> &piles) {
    ...
    for (int i = 1; i < piles.size(); ++i)
      for (int j = 0; j < piles.size() - i; ++j)
        dp[j][i + j] =
            max(piles[j] - dp[j + 1][i + j], piles[i + j] - dp[j][i + j - 1]);
