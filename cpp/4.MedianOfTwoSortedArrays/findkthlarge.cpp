...
public:
    double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
        ...
        if(((n1+n2)&1)==0)
            return (findKthBig(nums1,n1,nums2,n2,(n1+n2)/2)+findKthBig(nums1,n1,nums2,n2,(n1+n2)/2+1))/2.0;
        return findKthBig(nums1,n1,nums2,n2,(n1+n2)/2+1);
    }
    int findKthBig(vector<int>&nums1,int end1,vector<int>&nums2,int end2,int k){
        if(end1>end2)
