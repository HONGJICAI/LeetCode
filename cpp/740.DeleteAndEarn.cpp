...
public:
    int deleteAndEarn(vector<int>& nums) {
        ...
        int ret=0;
        function<void(itr,itr,int)> dfs=[&dfs,&used,&ret](itr first,itr last,int sum){
            if(first==last){
                ret=max(ret,sum);
                return;
            }
            if(not used.count(first->first+1) and not used.count(first->first-1)){
...
class Solution {
//Runtime: 8 ms, faster than 94.04% of C++ online submissions for Delete and Earn.
//Memory Usage: 11.2 MB, less than 31.37% of C++ online submissions for Delete and Earn.
public:
    int deleteAndEarn(vector<int>& nums) {
        int n = 10001;
