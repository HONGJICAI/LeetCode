class Solution {
    int row,col;
    bool search(string &word,int index,vector<vector<char>>&board,int i,int j,vector<vector<int>>&visited){
        if(i<0||i>=row||j<0||j>=col||visited[i][j]||word[index]!=board[i][j])
            return false;
        visited[i][j]=true;
