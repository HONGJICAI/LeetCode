...
class Solution {
//Runtime: 4 ms, faster than 88.22% of C++ online submissions for Integer Replacement.
//Memory Usage: 13 MB, less than 5.34% of C++ online submissions for Integer Replacement.
    unordered_map<long long, int> visited;
    int compute(long long n) {        
        if (n == 1) return 0;
        auto it=visited.find(n);
        if (it == end(visited))
            return (n & 1 == 1)?visited[n]=1+min(compute(n+1),compute(n-1)):visited[n]=1+compute(n/2);
        return it->second;
    }
public:
    int integerReplacement(int n) {        
